...
for i in xrange(epoch):
    print codes.shape, partial_captions.shape, next_words.shape
    ...
    for code, comment, raw in zip(codesL, commentsL, raw_commentL)[:]:
        sentences = gen.generateSentence(code, 10,nonUNK=True,useMemory=True)
    ...
    if max_score < score:
        ...
        open('../model/attmem_new.json', 'w').write(json_string)
        model.save_weights('../model/attmem_new.h5')
